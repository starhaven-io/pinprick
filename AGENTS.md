...
4. Keep action identities exact: a root action verdict does not cover subpaths or siblings. Local cache verdicts require the current scanner, detection-rules, and cache policy versions and the default runtime trust policy; configured host/data exemptions must not become reusable default-policy verdicts.
...
7. Keep SARIF rule IDs stable. Bump the audited-actions detection-rules version whenever detection or suppression semantics could invalidate a catalog verdict, then re-verify and restamp affected entries. When a change can invalidate only local cache verdicts, bump the local cache policy version instead. Change rubric versions deliberately when adjusting scoring semantics; document each contract alongside its implementation.
