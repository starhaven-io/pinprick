...
for JSON_FILE in "${FILES[@]}"; do
  ...
  if ! ENTRIES=$(jq -c "${SELECT}" "${JSON_FILE}"); then
    echo "::error::${OWNER}/${REPO} could not be parsed; entries NOT verified"
    FAILED=1
    continue
  fi
  ...
  while IFS= read -r ENTRY; do
    ...
    if [[ "${MODE}" == "shard" ]]; then
      ...
      HASH=$(cksum <<< "${OWNER}/${REPO}@${SHA}" | cut -d' ' -f1)
...
YAML
    ...
    if [[ "${STATUS}" -eq 1 ]]; then
      echo "::error::${OWNER}/${REPO}@${SHA} (${TAG}) has audit findings under the current rules"
      ...
      FAILED=1
    elif [[ "${STATUS}" -ne 0 ]] || [[ "$(jq -r '.scanned_fresh' <<< "${OUTPUT}")" != "1" ]]; then
      ...
      # "verify" everything.
      echo "::error::${OWNER}/${REPO}@${SHA} (${TAG}) could not be scanned (exit ${STATUS}); verdict NOT verified"
      FAILED=1
    fi
  done <<< "${ENTRIES}"
done
